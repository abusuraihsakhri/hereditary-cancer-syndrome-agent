import pytest
from hereditary_cancer import PedigreeCriteriaEvaluatorAgent, VariantPathogenicityAgent, SurveillanceProtocolAdvisorAgent, CancerGeneticsCoordinator, main


def test_sub_agents():
    a1 = PedigreeCriteriaEvaluatorAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = VariantPathogenicityAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = SurveillanceProtocolAdvisorAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = CancerGeneticsCoordinator()
...
def test_domain_registry():
    from hereditary_cancer import DomainKnowledgeRegistry
